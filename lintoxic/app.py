...
logging.basicConfig(filename="lintoxic-logs.log")
...
async def check_text(text: str):
    ...
    if accuracy.false_information:
        ...
        return (False, "The content is not harmful")
    ...
    if st.button("Check Content"):
        if uploaded_content:
            ...
            if result[0]:
                ...
                string = f"""
                <div style="background-color: #62e37988;padding: 16px;border-radius: 2em;text-align: center;">
                {result[1]}
                </div>
                """
                st.html(string)
